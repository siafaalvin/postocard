...
export function ViewCounter() {
  ...
  const disableNavCSS = `
    [data-bottomnav] a:not([href="/settings"]):not([href*="profile"]) {
      opacity: 0.3 !important;
      pointer-events: none !important;
    }
    [data-navbar] a:not([aria-label="user-menu"]), [data-navbar] button:not([aria-label="user-menu"]) {
      opacity: 0.3 !important;
      pointer-events: none !important;
    }
  `;
  ...
  if (atCap) {
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: disableNavCSS }} />
        <div className="fixed inset-0 top-[60px] z-[80] flex items-center justify-center bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md">
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-24 right-5 z-[60] flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 shadow-md text-xs font-medium transition-all duration-300 ${color}${animating ? " scale-125 animate-wiggle" : ""}`}
        title={`${remaining} of ${total} views remaining`}
      >
        <Eye size={12} />
        <span>{remaining}</span>
      </button>
