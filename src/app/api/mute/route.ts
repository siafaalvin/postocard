...
export async function POST(req: NextRequest) {
  ...
  const mute = await prisma.mute.upsert({
    ...
    create: { muterId: session.user.id, mutedId: targetUserId, contextNote, contextHashtag, visibleToTarget },
  });

  await prisma.userEvent.create({
    data: { userId: session.user.id, eventType: "muted", targetUserId },
  });
